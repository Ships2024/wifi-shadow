An idea from Sandman: Include "Evil Twin" attack in wifi-shadow.
...
Below are all of the requirements/components that wifi-shadow would need for this feature.
...
I think MDK is a better tool for this job, but wifi-shadow already requires the `aircrack` suite, so we should support both.
