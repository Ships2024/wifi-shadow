...
I can rely on `pip` + `requirements.txt` for python libraries, but most of wifi-shadow's dependencies are installed programs.

When a dependency is not found, wifi-shadow should walk the user through installing all required dependencies, and maybe the optional dependencies as well.
...
Since we have a Python script for every dependency (under `wifi-shadow/tools/` or `wifi-shadow/util/`), we use Python's multiple-inheritance to achieve this.
...
3. Dependency-checker to run at wifi-shadow startup.
...
* airmon-ng output differs, wifi-shadow sees "phy0" instead of the interface name.
...
* Some people want wifi-shadow to kill network manager, others don't.
...
wifi-shadow is a 'Spray and Pray', 'Big Red Button' script. wifi-shadow should not provide obscure options that only advanced users can understand. Advanced users can simply use wifi-shadow's dependencies directly.

--------------------------------

Every option in wifi-shadow's should either:

1. Significantly affect how wifi-shadow behaves (e.g. `pillage`, `5ghz`, '--no-wps', '--nodeauths')
...
Some dependencies of wifi-shadow (aircrack suite, tshark, etc) could be replaced with native Python implementations.
...
Some of wifi-shadow's dependencies work on other OSes (airodump) but some don't (airmon).

If it's possible to run these programs on Windows or OSX, wifi-shadow should support that.

------------------------------------------------------

### WPS Attacks

wifi-shadow's Pixie-Dust attack status output differs between Reaver & Bully. And the command line switches are... not even used by bully?
...
And as for PIN cracking.. um.. Not even sure this should be an option in wifi-shadow TBH.
PIN cracking takes days and most APs auto-lock after 3 attempts.
Multi-day (possibly multi-month) attacks aren't a good fit for wifi-shadow.
