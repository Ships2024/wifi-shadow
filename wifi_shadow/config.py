...
class Configuration(object):
    ''' Stores configuration variables and functions for wifi-shadow. '''
    version = '3.0.0'
    ...
    def initialize(cls, load_interface=True):
        ...
        cls.wps_seed_pins = True         # Try BSSID-derived seed PINs before full sweep
        cls.wps_pbc     = True           # Try WPS Push-Button (PBC) when PBC window active

        # Active PMKID harvest (scapy-based; falls back to hcxdumptool)
        cls.use_active_pmkid = True      # Attempt active PMKID harvest before passive
        cls.active_pmkid_attempts = 3    # Auth→Assoc attempts before giving up

        # Targeted hidden-AP decloak via sibling-SSID probes
        cls.targeted_decloak = True      # Enable active decloak for hidden APs
    ...
    def load_from_arguments(cls):
        ...
        cls.parse_wifi_shadow_args(args)
    ...
    def parse_wpa_args(cls, args):
        ...
        if args.wordlist:
            if not os.path.exists(args.wordlist):
                cls.wordlist = None
                Color.pl('{+} {C}option:{O} wordlist {R}%s{O} was not found, wifi-shadow will NOT attempt to crack handshakes' % args.wordlist)
                ...
                cls.wordlist = None
                Color.pl('{+} {C}option:{O} wordlist {R}%s{O} is a directory, not a file. wifi-shadow will NOT attempt to crack handshakes' % args.wordlist)
    ...
    @classmethod
    def parse_wifi_shadow_args(cls, args):
        '''Parses wifi-shadow specific arguments (active PMKID, seed PINs, PBC, decloak).'''
        if getattr(args, 'no_active_pmkid', False):
            cls.use_active_pmkid = False
            Color.pl('{+} {C}option:{W} {O}disabled{W} active PMKID harvest (scapy)')

        if getattr(args, 'no_wps_pbc', False):
            cls.wps_pbc = False
            Color.pl('{+} {C}option:{W} {O}disabled{W} WPS PBC attack')

        if getattr(args, 'no_wps_seed', False):
            cls.wps_seed_pins = False
            Color.pl('{+} {C}option:{W} {O}disabled{W} BSSID-derived seed PINs')

        if getattr(args, 'no_decloak', False):
            cls.targeted_decloak = False
            Color.pl('{+} {C}option:{W} {O}disabled{W} targeted decloak probes')

        if getattr(args, 'active_pmkid_attempts', None):
            cls.active_pmkid_attempts = args.active_pmkid_attempts
            Color.pl('{+} {C}option:{W} active PMKID harvest attempts set to {G}%d{W}' %
                     cls.active_pmkid_attempts)
