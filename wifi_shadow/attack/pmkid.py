...
from ..util.hc22000 import save_hc22000_for_handshake
...
class AttackPMKID(Attack):
    ...
    def run(self):
        ...
        Performs PMKID attack, if possible.
            1) Tries active PMKID harvest (scapy Auth→Assoc→M1) if enabled.
            2) Falls back to passive hcxdumptool capture.
            3) Cracks the captured hash.
        ...
        if pmkid_file is None and Configuration.use_active_pmkid:
            # Try active PMKID harvest first (no external tools needed beyond scapy)
            pmkid_file = self._try_active_pmkid()

        if pmkid_file is None:
            # Fall back to passive hcxdumptool capture
            dependencies = [
            ...
            if len(missing_deps) > 0:
                Color.pl('{!} Skipping passive PMKID capture, missing tools: {O}%s{W}' % ', '.join(missing_deps))
                return False
    ...
    def _try_active_pmkid(self):
        '''
        Attempt active PMKID harvest via scapy (Auth→Assoc→EAPOL M1).
        Returns path to saved .hc22000 file on success, None otherwise.
        '''
        try:
            from ..attack.pmkid_active import AttackPMKIDActive
        except ImportError:
            return None

        if not AttackPMKIDActive.is_available():
            return None

        harvester = AttackPMKIDActive(
            self.target,
            attempts=Configuration.active_pmkid_attempts,
        )
        if harvester.run():
            return harvester.save(Configuration.wpa_handshake_dir)
        return None
    ...
    def crack_pmkid_file(self, pmkid_file):
        ...
        if Configuration.wordlist is None:
            ...
            #Color.pl('{!} {O}Run wifi-shadow with the {R}--crack{O} and {R}--dict{O} options to try again.')
    ...
    def save_pmkid(self, pmkid_hash):
        ...
        # Also save hashcat -m 22000 format (.hc22000) for newer hashcat
        hc22000_file = pmkid_file.replace('.16800', '.hc22000')
        try:
            from ..util.hc22000 import cap_to_hc22000
            # hcxpcapngtool/hcxpcaptool not applicable here (no cap file) — skip
        except ImportError:
            pass
