...
class Arguments(object):
    ''' Holds arguments used by WifiShadow '''
    ...
    def get_arguments(self):
        ...
        self._add_wifi_shadow_args(parser.add_argument_group(Color.s('{C}ENHANCED (wifi-shadow){W}')))
    ...
    def _add_wifi_shadow_args(self, group):
        '''wifi-shadow specific enhancements.'''
        group.add_argument('--no-active-pmkid',
            action='store_true',
            dest='no_active_pmkid',
            help=Color.s('Disable {C}active PMKID harvest{W} (scapy Auth→Assoc→M1); '
                         'use passive {C}hcxdumptool{W} only (default: {G}off{W})'))

        group.add_argument('--active-pmkid-attempts',
            action='store',
            dest='active_pmkid_attempts',
            metavar='[num]',
            type=int,
            default=None,
            help=self._verbose('Number of Auth→Assoc attempts for active PMKID '
                               '(default: {G}3{W})'))

        group.add_argument('--no-wps-pbc',
            action='store_true',
            dest='no_wps_pbc',
            help=Color.s('Disable {C}WPS PBC{W} (Push-Button) attack (default: {G}off{W})'))

        group.add_argument('--no-wps-seed',
            action='store_true',
            dest='no_wps_seed',
            help=self._verbose('Disable {C}BSSID-derived seed PINs{W} for WPS PIN attack '
                               '(default: {G}enabled{W})'))

        group.add_argument('--no-decloak',
            action='store_true',
            dest='no_decloak',
            help=self._verbose('Disable {C}targeted sibling-SSID decloak{W} probes for '
                               'hidden APs (default: {G}enabled{W})'))
