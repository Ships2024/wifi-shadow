...
try:
    ...
    _DECLOAK_AVAILABLE = True
except ImportError:
    _DECLOAK_AVAILABLE = False
        ...
        try:
            with Airodump() as airodump:
                ...
                while True:
                    ...
                    # Active targeted decloak for hidden APs via sibling-SSID probes
                    if _DECLOAK_AVAILABLE and Configuration.targeted_decloak:
                        for target in self.targets:
                            if not target.essid_known and not target.decloaked:
                                try:
                                    _decloak_attempt(target, self.targets)
                                except Exception:
                                    pass
