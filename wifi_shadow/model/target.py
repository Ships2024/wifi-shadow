...
class Target(object):
    ...
    def __init__(self, fields):
        ...
        # WPA3/PMF/AKM attributes (populated by rsn_parser when RSN IE is available)
        self.akm_suites        = []    # List of AKM suite integers
        self.pmf_capable       = False # Management Frame Protection capable
        self.pmf_required      = False # Management Frame Protection required
        self.has_psk_akm       = True  # Has WPA2-PSK AKM (default True if unknown)
        self.has_sae_akm       = False # Has SAE (WPA3-Personal) AKM
        self.has_wpa3          = False # Pure WPA3 (SAE-only)
        self.has_wpa3_transition = False # WPA3-Transition (SAE + PSK)

        # WPS PBC flag (set by scanner when Selected Registrar=1 seen in beacons)
        self.wps_pbc_active    = False
    ...
    def to_str(self, show_bssid=False):
        ...
        # Build encryption label with optional WPA3/PMF suffix
        enc_label = self.encryption
        if self.has_wpa3 and not self.has_wpa3_transition:
            enc_label = 'WPA3'
        elif self.has_wpa3_transition:
            enc_label = 'WP3T'  # WPA3-Transition (4 chars)
        encryption = enc_label.rjust(4)
        if 'WEP' in encryption:
            encryption = Color.s('{G}%s' % encryption)
        elif 'WPA' in encryption or 'WP3' in encryption:
            if self.has_wpa3 or self.has_wpa3_transition:
                encryption = Color.s('{C}%s' % encryption)  # Cyan for WPA3
            else:
