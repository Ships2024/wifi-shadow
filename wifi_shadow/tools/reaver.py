...
# Optional BSSID-derived seed PINs
try:
    from ..tools.wps_pins import seed_pins_for as _seed_pins_for
    _SEED_PINS_AVAILABLE = True
except ImportError:
    _SEED_PINS_AVAILABLE = False
...
class Reaver(Attack, Dependency):
    ...
    def _get_seed_pins(self):
        '''Returns BSSID-derived seed PINs if enabled, else empty list.'''
        if not _SEED_PINS_AVAILABLE:
            return []
        if not Configuration.wps_seed_pins:
            return []
        if self.pixie_dust:
            return []  # Pixie-dust doesn't use PINs
        try:
            pins = _seed_pins_for(self.target.bssid)
            if pins and Configuration.verbose > 0:
                Color.pl('{+} {C}Seed PINs:{W} trying {G}%d{W} BSSID-derived PINs first'
                         % len(pins))
            return pins
        except Exception:
            return []

    def _try_seed_pin(self, pin, airodump):
        '''Run reaver with a single --pin value. Returns True if cracked.'''
        cmd = self.reaver_cmd + ['--pin', pin]
        # Brief timeout — just enough for a single exchange
        proc = Process(cmd, stdout=self.output_write, stderr=Process.devnull())
        proc.stdin('y\n')

        deadline = time.time() + 30  # 30 s per seed PIN attempt
        while proc.poll() is None and time.time() < deadline:
            stdout = self.get_output()
            self.pattack('Seed PIN {C}%s{W} ...' % pin)
    ...
    def _run(self):
        ...
        with Airodump(channel=self.target.channel,
            ...
            # Try BSSID-derived seed PINs before full sweep (PIN attack only)
            seed_pins = self._get_seed_pins()
            for pin in seed_pins:
                if self._try_seed_pin(pin, airodump):
                    return  # Cracked via seed PIN
